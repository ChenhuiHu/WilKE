...
class EFK(EditableModel):
    ...
    def __init__(self, model, config, editor=None):
    ...
    def edit(self, batch, condition, detach_history=False):     
        ...
        #if not isinstance(self.model, higher.patch._MonkeyPatchBase):
        #    self.model = make_functional(self.model, in_place=True)
        ...
        #    [new_param(n, p) for (n, p) in self.model.named_parameters()]
        #)
        
        for (name, param) in [(n, new_param(n, p)) for (n, p) in self.model.named_parameters()]:
        ...
        #print("model target same:",id(edited_model) == id(self.model))
        #return edited_model
        return self.model
